...
namespace DigitalRuby.PyroParticles
{
    ...
    public class FireProjectileScript : FireBaseScript, ICollisionHandler
    {
        ...
        public void HandleCollision(GameObject obj, Collision c)
        {
            ...
            {
                //moves the poof to align with the projectile
				ProjectileExplosionParticleSystem.transform.position = c.contacts[0].point;
               
				//plays the poof of the projectile
            ...
            EyeBotBehavior eyeBotBehavior = c.gameObject.GetComponent<EyeBotBehavior>();
            ObjectHealth objectHealth = c.gameObject.GetComponent<ObjectHealth>();
            PowerModuleHealth powerModuleHealth = c.gameObject.GetComponent<PowerModuleHealth>();
            GunBotHealth gunBotHealth = c.gameObject.GetComponent<GunBotHealth>();
            RedRobotEnemyHealth redRobotEnemyHealth = c.gameObject.GetComponent<RedRobotEnemyHealth>();
            BossModuleHealth bossModuleHealth = c.gameObject.GetComponent<BossModuleHealth>();
            BossBotBehavior bossBotBehavior = c.gameObject.GetComponent<BossBotBehavior>();
            ...
            {
                objectHealth.TakeDamage(100, c.transform.position);
            }


            if (powerModuleHealth != null)
            {
                powerModuleHealth.TakeDamage(100, c.transform.position);
            }

            if (bossModuleHealth != null)
            {
                bossModuleHealth.TakeDamage(100, c.transform.position);
            }

            if (bossBotBehavior != null)
            {
                bossBotBehavior.TakeDamage(100, c.transform.position);
            }

            if (gunBotHealth != null)
            {
                gunBotHealth.TakeDamage(100, c.transform.position);
            }

            if (redRobotEnemyHealth != null)
            {
                redRobotEnemyHealth.TakeDamage(100, c.transform.position);
            }

            if (eyeBotBehavior != null)
            {
                eyeBotBehavior.TakeDamage(100, c.transform.position);
            }
